#!/usr/bin/env bash
# Provoca a recuperação: standoff_lateral 1,8 m só na etapa intermediária
...
OUT=/home/marco/.claude/jobs/89ade7b6/tmp/teste_recuperacao2
mkdir -p "$OUT"
source /home/marco/miniforge3/etc/profile.d/conda.sh; conda activate ros_env; source /home/marco/b166er/devel/setup.bash
rosparam set /chave_mission/standoff_lateral 1.8
for n in 1 2; do
  "$R" "$n" "$OUT" 0.0 1.0 0 "" 2>&1 | tail -2
  sleep 5
done
rosparam delete /chave_mission/standoff_lateral
echo "[recup2] fim $(date +%H:%M:%S)"

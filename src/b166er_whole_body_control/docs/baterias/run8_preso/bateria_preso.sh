...
for i in $(seq 1 $N); do
  ...
  sleep 2
  rosparam set /chave_mission/ensaio_forca_preso true
  LOG=$OUT/preso_run$i.log
  ...
  echo "[preso run$i] $(grep -aE 'FORÇADO|REASSENTANDO|reassenta: captura|destrava: ponta desceu|libera.*alcançada|SOLTURA|lâmina medida em|não conseguiu|recaptura não|não fechou de novo|resultado: MISSION' $LOG | sed 's/.*\[mission\] //' | cut -c1-80 | tr '\n' ';')"
done
rosparam set /chave_mission/ensaio_forca_preso false
echo "[bat] fim"

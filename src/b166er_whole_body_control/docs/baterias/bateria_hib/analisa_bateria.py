...
def analisa(run):
    ...
    for l in run["linhas"]:
        ...
        m = re.search(r'fase "(\w+)" alcançada por whole-body em ([\d.]+)s \(([\d.]+) m\)(?: — no frame da parede eixo=([-+\d.]+) prof=([-+\d.]+) alt=([-+\d.]+))?', l)
        if m:
            eixos = ("" if m.group(4) is None else " [%s/%s/%s]" % (m.group(4), m.group(5), m.group(6)))
            r["fases"][m.group(1)] = (float(m.group(2)), 1000 * float(m.group(3)), "wb" + eixos)
...
def tabela(rotulo, runs, sondas):
    ...
    for nome, run in runs:
        ...
        eixos = {f: v[2] for f, v in a["fases"].items() if isinstance(v[2], str) and "[" in v[2]}
        print("%-6s %-14s %6s %5.0f | %s" % (nome, a["resultado"], "%.1f°" % a["lamina"] if a["lamina"] else "-", a["dur"], " ".join(cel)))
        if eixos:
            print("       por eixo ao fechar (eixo/prof/alt mm): %s" % "  ".join("%s %s" % (f[:5], eixos[f].split("wb ")[1]) for f in FASES if f in eixos))
        s = sondas.get(nome)
        if s:
            lmax, nz, n, pf = s
...
import os
D = sys.argv[1] if len(sys.argv) > 1 else "."; N0 = int(sys.argv[2]) if len(sys.argv) > 2 else 21
rosout = sorted(glob.glob(os.path.join(D, "run*_rosout_acumulado.log")))[-1]
wb = runs_de(rosout)
ULT = int(sys.argv[3]) if len(sys.argv) > 3 else len(wb)
wb = wb[-ULT:]
sondas = {"run%d" % (N0 + k): sonda(os.path.join(D, "run%d_sonda.csv" % (N0 + k))) for k in range(len(wb))}
tabela("WHOLE-BODY (use_wholebody:=true), a partir da run %d — %s" % (N0, rosout), [("run%d" % (N0 + k), r) for k, r in enumerate(wb)], sondas)
ik = runs_de("/home/marco/.claude/projects/-home-marco-b166er/sessoes/2026-09-03_ferramentas/run19_run20_rosout.log")
tabela("POSTURA (IK iterativa), runs 18-20 (mesmo fixture com gatilho; a 18 abortou pela barra)", [("run%d" % (18 + k), r) for k, r in enumerate(ik)], {})
